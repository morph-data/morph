...
class ApiTask(BaseTask):
    def __init__(self, args: Flags):
        ...
        # port
        self.server_port = self._find_available_port(9002)
        ...
        self.is_production = args.PRODUCTION or False
    ...
    def _run_frontend(self) -> None:
        ...
        if not os.path.exists(frontend_dir):
            click.echo(
                click.style(
                    f"Error: Frontend directory not found at {frontend_dir}.",
                    fg="red",
                )
            )
            sys.exit(1)
        ...
        if self.is_production:
            ...
            self._run_process(["npm", "run", "dev"], cwd=frontend_dir)
